#!/bin/bash
# Skrypt instalacyjny dla Ultra Defender for Linux
...
if [ "$(id -u)" -ne 0 ]; then
    ...
    sudo bash "${CONFIG_SCRIPT}" # Wywołanie w trybie interaktywnym
else
    bash "${CONFIG_SCRIPT}" # Wywołanie w trybie interaktywnym
fi

echo "Proces instalacji/konfiguracji zakończony."